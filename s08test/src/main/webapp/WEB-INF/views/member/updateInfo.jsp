...
<html>
	...
	<body>
		<div>
			<h1>회원가입</h1>
			<form action="/member/updateInfo" name="m_frm" method="post">
			<table>
				<tr>
					<th>아이디</th>
				...
				<tr>
					...
					<td>
						...
						<c:if test="${member.hobbys.contains('game') }"> checked</c:if>
						>
						<label for="game">게임</label>
						<input type="checkbox" name="hobby" id="golf" value="golf"
						<c:if test="${member.hobbys.contains('golf') }"> checked</c:if>
						>
						<label for="golf">골프</label>
						<input type="checkbox" name="hobby" id="run" value="run"
						<c:if test="${member.hobbys.contains('run') }"> checked</c:if>
						>
						<label for="run">조깅</label>
						<input type="checkbox" name="hobby" id="cook" value="cook"
						<c:if test="${member.hobbys.contains('cook') }"> checked</c:if>
						>
						<label for="cook">요리</label>
						<input type="checkbox" name="hobby" id="book" value="book"
						<c:if test="${member.hobbys.contains('book') }"> checked</c:if>
						>
